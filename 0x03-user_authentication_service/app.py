...
def register_user() -> str:
    ...
    messg = {"email": email, "message": "user created"}
    return jsonify(messg)
...
def log_in() -> str:
    ...
    messg = {"email": email, "message": "logged in"}
    response = jsonify(messg)
...
def profile() -> str:
    ...
    return jsonify(messg), 200


@app.route('/reset_password', methods=['POST'])
def reset_password() -> str:
    """ email is not registered, respond with a 403 status code.
    ...
    messg = {"email": email, "reset_token": reset_token}
...
def update_password() -> str:
    ...
    messg = {"email": email, "message": "Password updated"}
    return jsonify(messg), 200
